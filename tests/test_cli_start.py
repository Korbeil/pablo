...
def make_jira_cfg(tmp_path: Path, name: str = "acme-oms") -> ProjectConfig:
    return ProjectConfig(
        name=name,
        type="work",
...
def jira_env(env, tmp_path, monkeypatch):
    ...
    projects = {"wallet-kit": env["cfg"], "acme-oms": jira_cfg}
...
def test_start_prompt_with_issue_key_resolves_issue(jira_env, capsys):
    rc = cli.main(
        ["start", "--project", "acme-oms", "fix the thing per OMS-6393 please"]
    )
    assert rc == 0
    assert jira_env["created"] == [("oms-6393", "main")]
    task = jira_env["store"]().get("acme-oms", "oms-6393")
    assert task is not None
