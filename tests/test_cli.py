...
def test_internal_launch_agent_dispatches_to_do_launch_agent(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        agents, "_refresh_agent_display_cache",
        lambda project, branch, worktree: None,
    )
    args = argparse.Namespace(
        worktree=str(tmp_path), agent="task-analyst", prompt="hello",
        project="test-proj", branch="test-branch",
    )
...
def test_internal_run_startup_script_dispatches_to_do_run_startup_script(monkeypatch, tmp_path):
    ...
    args = argparse.Namespace(
        worktree=str(tmp_path), script=str(script),
        project="test-proj", branch="test-branch",
    )
...
def test_internal_parser_parses_launch_agent():
    ...
    args = parser.parse_args(
        ["internal-launch-agent", "--worktree", "/wt", "--agent", "task-analyst",
         "--prompt", "hi", "--project", "proj", "--branch", "br"]
    )
