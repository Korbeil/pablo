...
def cfg(tmp_path: Path, site: str | None = "acme.atlassian.net",
...
def test_match_url_extracts_id_from_query():
    url = "https://acme.atlassian.net/wiki?pageId=36307094"
    assert confluence.match_url(url) == "36307094"


def test_match_url_returns_none_for_non_confluence():
    assert confluence.match_url("https://acme.atlassian.net/browse/PIM-1") is None
...
def test_fetch_by_bare_id(tmp_path, monkeypatch):
    ...
    assert page.url == "https://acme.atlassian.net/wiki/spaces/PIM/overview"
...
def test_fetch_by_url_parses_id(tmp_path, monkeypatch):
    ...
    page = confluence.fetch(
        "https://acme.atlassian.net/wiki/spaces/PIM/pages/36307094/PIM+Home",
        cfg(tmp_path),
    )
...
def test_fetch_rejects_unparseable_url(tmp_path):
    with pytest.raises(PabloError, match="not a page id or Confluence URL"):
        confluence.fetch("https://acme.atlassian.net/browse/PIM-1", cfg(tmp_path))
