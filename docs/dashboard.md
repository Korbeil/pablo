...
Both halves come from `Listing::isWaitingForFeedback()`, the *same*
predicate the terminal listing splits on, so `pablo tasks` and this page
can never disagree. Both tables then sort by `Listing::stateRank()` then
...
Unlike the CLI, the web page keeps both headings even when the first
section is empty — a table needs its header, and the empty state reads
