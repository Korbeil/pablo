---
description: Liste Slack des PR à tester, groupées par projet
subtask: true
---

## Context (auto-injected)

!`pablo queue needs-testing`

## Your task

The JSON array above lists every PABLO task currently in the
`needs-testing` (🧪) state, across all configured projects, each with its
...
- If a task has no `pr` (no PR yet), skip that bullet — it's not
  something a tester can act on. If that empties a project's whole
...
- If the array is empty, or nothing has a PR, output exactly:
  `Nothing needs testing right now 🎉`
